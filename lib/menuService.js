...
/**
 * checkItemsAvailability(itemIds)
 * One-time (not live) re-check of whether each itemId is still orderable
 * right now — called at checkout, right before submitOrder, to catch an
 * item that went unavailable (86'd, or soft-deleted) while sitting in a
 * customer's cart. The live subscription in MenuView only catches this if
 * the customer is still looking at the menu; it never re-validates
 * what's already sitting in the cart.
 *
 * Returns the subset of itemIds that are NOT orderable (missing, soft-
 * deleted, or available === false — same semantics as itemDocToEntry).
 */
export async function checkItemsAvailability(itemIds) {
  const uniqueIds = [...new Set(itemIds)].filter(Boolean);
  if (uniqueIds.length === 0) return [];

  const snaps = await Promise.all(uniqueIds.map((id) => getDoc(doc(db, "menuItems", id))));

  return uniqueIds.filter((id, i) => {
    const snap = snaps[i];
    if (!snap.exists()) return true;
    const d = snap.data();
    return d.deletedAt || d.available === false;
  });
}
