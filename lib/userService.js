...
//
//   users/{uid}
//     email, role: "admin" | "staff" | "kitchen", venueId, createdAt
//
// Role semantics (must match firestore.rules — isFOH/isVenueStaff/isKitchen):
//   "admin"   — full menu CRUD (categories/items) + can create staff/kitchen
//               accounts for their OWN venue. Created automatically as the
//               owner when a venue is created (see createVenueWithAdmin).
//   "staff"   — front-of-house: can view/edit orders and bills, cannot edit
//               the menu. (Formerly split into "manager"/"server", which
//               were always identical in permissions — collapsed into one
//               role to match firestore.rules' isFOH().)
//   "kitchen" — Kitchen Display System only; may flip a menu item's
//               `available` flag and change an order's status, nothing else.
//
...
// creating a Staff/Kitchen account FROM WITHIN their own session (it would
 ...
 * Called by an Admin to create a Staff or Kitchen account FOR THEIR OWN
 * venue. Uses a secondary, temporary Firebase Auth app instance so the
 * Admin's own session in the main `auth` instance is untouched — without
 * this, signing up a new user would silently log the Admin out and log
 * the new staff member in instead.
 */
export async function createStaffAccount({ email, password, role, venueId }) {
  if (role !== "staff" && role !== "kitchen") {
    throw new Error('role must be "staff" or "kitchen"');
  }
 ...
 * Lists every user with role "staff" or "kitchen" (NOT admin) belonging
 * to a venue — powers the "Staff" tab in the admin menu editor.
 */
export function subscribeToVenueStaff(venueId, callback) {
  ...
  return onSnapshot(
    ...
    (snap) => {
      ...
      const staff = all.filter((u) => u.role === "staff" || u.role === "kitchen");
